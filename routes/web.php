...
Route::get('/get-groups/{companyId}', function ($companyId) {
    ...
    if ($company) {
        $groups = $company->groups; // Get the groups associated with the selected company
        return response()->json(['groups' => $groups]);
    } else {
        return response()->json(['groups' => []]);
    }
});
Route::get('/group/{group}/users', [GroupController::class, 'getUsers']);

Route::any('/cookie-consent', [SettingsController::class,'CookieConsent'])->name('cookie-consent');


Route::get('/review/{ticket_id}', [ReviewController::class,'showForm'])->name('review.form');
Route::post('/review/{ticket_id}', [ReviewController::class,'store']);

Route::post('/mailgun/webhook', [IncomingEmailController::class,'handleWebhook']);
...
Route::name('admin.')->prefix('admin')->middleware(['auth','XSS','role:admin'])->group(function() {
    ...
    //notification//
     Route::get('mark_all_read',  [NotificationTemplatesController::class, 'markAllRead'])->name('notifications.markAllRead');
     Route::get('mark_as_read/{id}', [NotificationTemplatesController::class, 'markAsRead'])->name('notifications.markAsRead');
     Route::get('notifications/fetch', [NotificationTemplatesController::class, 'fetch'])->name('notifications.fetch');
...
Route::name('admin.')->prefix('admin')->middleware(['auth','XSS'])->group(function() {
    ...
    Route::get('fetch_review', [ReviewController::class, 'fetchReviews'])->name('reviews.fetchReviews');
    ...
    Route::get('check-lock-status/{ticketId}', [TicketController::class, 'checkLockStatus']);
    Route::get('take_ticket/{ticketId}', [TicketController::class, 'take_ticket']);
